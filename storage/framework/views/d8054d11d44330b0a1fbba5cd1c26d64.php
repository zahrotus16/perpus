...
<?php $__env->startSection('page-title', 'Pusat Kendali'); ?>
<?php $__env->startSection('page-subtitle', 'Ikhtisar aktivitas dan status literasi Anda'); ?>
...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-8 mb-8">
        <div>
            ...
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Halo, <?php echo e(explode(' ', auth()->user()->name)[0]); ?>!</h1>
            <p class="text-slate-500 text-sm mt-1 font-medium italic">Status: <?php echo e(auth()->user()->role_label); ?> • ID: <?php echo e(auth()->user()->member_id); ?></p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="group relative bg-white p-6 rounded-2xl shadow-sm border border-slate-100 transition-all hover:shadow-xl hover:-translate-y-1 overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-baseline gap-2">
                    <h3 class="text-2xl font-black text-slate-900 tracking-tight"><?php echo e(sprintf("%02d", $stats['active_loans'])); ?></h3>
        ...
        <div class="group relative bg-white p-6 rounded-2xl shadow-sm border border-slate-100 transition-all hover:shadow-xl hover:-translate-y-1 overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-baseline gap-2">
                    <h3 class="text-2xl font-black text-slate-900 tracking-tight"><?php echo e(sprintf("%02d", $stats['total_loans'])); ?></h3>
        ...
        <div class="group relative bg-white p-6 rounded-2xl shadow-sm border border-slate-100 transition-all hover:shadow-xl hover:-translate-y-1 overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-baseline gap-2">
                    <h3 class="text-2xl font-black text-slate-900 tracking-tight">Rp <?php echo e(number_format($stats['total_fine'], 0, ',', '.')); ?></h3>
                </div>
            </div>
        </div>
        ...
        <div class="group relative bg-white p-6 rounded-2xl shadow-sm border border-slate-100 transition-all hover:shadow-xl hover:-translate-y-1 overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-baseline gap-2">
                    <h3 class="text-2xl font-black text-slate-900 tracking-tight"><?php echo e(sprintf("%02d", auth()->user()->wishlists()->count())); ?></h3>
    ...
    <div class="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/20 rounded-2xl p-4 flex items-center gap-4 animate-pulse">
        ...
        <div class="flex-1">
            ...
            <p class="text-[10px] text-amber-700 dark:text-amber-500/80 font-bold">Anda memiliki <?php echo e($dueSoonLoans->count()); ?> buku yang hampir jatuh tempo. Segera lakukan pengembalian!</p>
        </div>
        <a href="<?php echo e(route('user.loans.active')); ?>" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-[9px] font-black rounded-lg uppercase tracking-widest transition-all shadow-lg shadow-amber-900/20">Cek Sekarang</a>
    </div>
    <?php endif; ?>
        ...
        <div class="xl:col-span-2 space-y-8">
            ...
            <div class="bg-white dark:bg-dark-card rounded-3xl shadow-sm dark:shadow-dark-soft border border-slate-100 dark:border-dark-border/20 overflow-hidden">
                <div class="px-8 py-5 border-b border-slate-50 dark:border-dark-border/10 flex items-center justify-between">
                    ...
                    <a href="<?php echo e(route('user.loans.index')); ?>" class="text-[10px] font-black text-primary-600 dark:text-primary-400 uppercase tracking-widest hover:underline transition-all">Manajemen →</a>
                </div>
                <div class="p-4 space-y-4">
                    ...
                    <div class="group flex items-center gap-6 p-4 rounded-2xl border border-slate-50 dark:border-dark-border/10 hover:bg-slate-50 dark:hover:bg-white/5 transition-all duration-300">
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="text-sm font-black text-slate-800 dark:text-white truncate uppercase tracking-tight mb-1"><?php echo e($loan->book->title); ?></h4>
                            <div class="flex items-center gap-3">
                                <span class="bg-slate-100 dark:bg-dark px-2 py-0.5 rounded text-[9px] font-black text-slate-500 dark:text-gray-500 uppercase tracking-widest border border-slate-200 dark:border-dark-border/10"><?php echo e($loan->book->category->name); ?></span>
                                <span class="text-[10px] font-black <?php echo e($loan->isOverdue() ? 'text-red-500 animate-pulse' : 'text-slate-400 dark:text-gray-600'); ?> uppercase">
                                    <i class="fas fa-clock mr-1.5 opacity-50"></i>
                                    <?php echo e($loan->tgl_kembali ? $loan->tgl_kembali->format('d M') : '-'); ?> </span>
                            </div>
                        </div>
                        <?php if($loan->book->pdf_file): ?>
                        <a href="<?php echo e(route('user.books.read', $loan->book)); ?>" class="flex-shrink-0 w-11 h-11 bg-primary-600 hover:bg-primary-700 text-white rounded-xl flex items-center justify-center shadow-lg shadow-primary-900/20 active:scale-90 transition-all">
            ...
            <div>
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="<?php echo e(route('user.books.index')); ?>" class="text-[10px] font-black text-slate-500 dark:text-gray-500 uppercase tracking-widest hover:text-primary-600 transition-colors">Lihat Katalog →</a>
                </div>
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-6">
                    ...
                    <a href="<?php echo e(route('user.books.show', $book)); ?>" class="group block bg-white dark:bg-dark-card rounded-2xl shadow-sm border overflow-hidden transition hover:-translate-y-2 hover:shadow-xl">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-slate-400 leading-relaxed mt-1">Anda memiliki <?php echo e($stats['active_loans']); ?> buku aktif. Terus tingkatkan intensitas baca harian.</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-slate-400 leading-relaxed mt-1">Terverifikasi sejak <?php echo e(auth()->user()->created_at->format('M Y')); ?>. Kontribusi signifikan terdeteksi.</p>
                            </div>
                        </div>
                    </div>
                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\perpustakaan-digital\resources\views/user/dashboard_anggota.blade.php ENDPATH**/ ?>