<div class="relative" id="profileDropdown">
    <button onclick="toggleDropdown(event)" class="flex items-center gap-3 p-1 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-all focus:outline-none group">
        ...
        <div class="hidden lg:flex flex-col items-start leading-tight">
            <span class="text-xs font-black text-slate-800 dark:text-white line-clamp-1 truncate max-w-[120px]"><?php echo e(auth()->user()->name); ?></span>
            <span class="text-[9px] font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest"><?php echo e(auth()->user()->role_label); ?></span>
        </div>
    ...
    <div id="dropdownMenu" class="hidden absolute right-0 mt-2 w-56 bg-white dark:bg-dark-card rounded-2xl shadow-xl dark:shadow-dark-soft border border-slate-100 dark:border-dark-border/30 p-2 z-[60] dropdown-animate">
        <div class="px-3 py-2 mb-2 border-b border-slate-50 dark:border-dark-border/20">
            <p class="text-xs font-bold text-slate-900 dark:text-white"><?php echo e(auth()->user()->name); ?></p>
            <p class="text-[10px] font-medium text-slate-500 dark:text-gray-400 truncate"><?php echo e(auth()->user()->email); ?></p>
        </div>
        <a href="<?php echo e(route('admin.settings.index')); ?>" class="flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
...
<?php /**PATH D:\perpustakaan-digital\resources\views/components/admin/profile-dropdown.blade.php ENDPATH**/ ?>