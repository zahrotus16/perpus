...
<?php $__env->startSection('page-subtitle', $loan->kode_peminjaman); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-3xl mx-auto space-y-6">

    <a href="<?php echo e(route('admin.loans.index')); ?>" class="inline-flex items-center gap-2 text-sm text-slate-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition font-bold uppercase tracking-wider text-[11px]">
    ...
    <div class="bg-white dark:bg-dark-card rounded-2xl shadow-sm dark:shadow-dark-soft border border-slate-100 dark:border-dark-border/20 overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 dark:border-dark-border/10 flex items-center justify-between bg-slate-50/50 dark:bg-dark/30">
            <div>
                ...
                <h2 class="text-2xl font-black text-slate-800 dark:text-white font-mono tracking-tighter"><?php echo e($loan->kode_peminjaman); ?></h2>
            </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-dark border border-slate-100 dark:border-dark-border/10">
                        <div class="w-12 h-12 rounded-full bg-gradient-to-br from-primary-400 to-purple-600 flex items-center justify-center text-white text-lg font-bold shadow-sm">
                            <?php echo e(strtoupper(substr($loan->user->name,0,1))); ?>

                        </div>
                        <div>
                            <p class="text-base font-bold text-slate-800 dark:text-white leading-none"><?php echo e($loan->user->name); ?></p>
                            <p class="text-xs text-slate-400 dark:text-gray-500 mt-1.5 font-mono"><?php echo e($loan->user->member_id); ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 rounded-2xl bg-slate-50 dark:bg-dark border border-slate-100 dark:border-dark-border/10">
                        <p class="text-base font-bold text-slate-800 dark:text-white leading-tight mb-1"><?php echo e($loan->book->title); ?></p>
                        <p class="text-xs text-slate-400 dark:text-gray-500 italic"><?php echo e($loan->book->author); ?></p>
                        <div class="mt-2 flex items-center gap-2">
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded-full ring-1 ring-inset ring-slate-200 dark:ring-dark-border/20 bg-white dark:bg-dark-card text-slate-500 dark:text-gray-400">
                                <?php echo e(optional($loan->book->category)->name ?? '-'); ?> </span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-6 py-6 border-t border-slate-100 dark:border-dark-border/10">
                <div>
                    ...
                    <p class="text-sm font-bold text-slate-700 dark:text-white"><?php echo e($loan->tgl_pinjam ? $loan->tgl_pinjam->format('d M Y') : '-'); ?></p>
                </div>
                <div>
                    <p class="text-[10px] text-slate-400 dark:text-gray-500 font-bold uppercase tracking-wider mb-1.5">Batas Kembali</p>
                    <p class="text-sm font-bold <?php echo e($loan->isOverdue() ? 'text-red-600 dark:text-red-400 underline underline-offset-4 decoration-red-500/30' : 'text-slate-700 dark:text-white'); ?>"><?php echo e($loan->tgl_kembali ? $loan->tgl_kembali->format('d M Y') : '-'); ?></p>
                </div>
                <div>
                    <p class="text-[10px] text-slate-400 dark:text-gray-500 font-bold uppercase tracking-wider mb-1.5">Dikembalikan</p>
                    <p class="text-sm font-bold text-slate-700 dark:text-white"><?php echo e($loan->pengembalian ? $loan->pengembalian->tgl_dikembalikan->format('d M Y') : '-'); ?></p>
                </div>
                <div>
                    <p class="text-[10px] text-slate-400 dark:text-gray-500 font-bold uppercase tracking-wider mb-1.5">Status Denda</p>
                    <p class="text-sm font-bold <?php echo e(($loan->pengembalian->denda ?? 0) > 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-400 dark:text-gray-600'); ?>">
                        <?php echo e(($loan->pengembalian->denda ?? 0) > 0 ? 'Rp '.number_format($loan->pengembalian->denda,0,',','.') : 'Nihil'); ?>
            ...
            <div class="mt-4 p-4 bg-red-50 dark:bg-red-900/20 rounded-2xl border border-red-100 dark:border-red-900/30 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-red-700 dark:text-red-300 font-bold">Total denda sebesar Rp <?php echo e(number_format($fine,0,',','.')); ?> telah diakumulasikan.</p>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="mt-8">
                ...
                <div class="text-sm text-slate-600 dark:text-gray-400 bg-slate-50 dark:bg-dark rounded-2xl p-4 border border-slate-100 dark:border-dark-border/10 italic leading-relaxed">
                    <?php echo e($loan->catatan); ?>

                </div>
            </div>
            <?php endif; ?>

            <?php if($loan->status !== 'dikembalikan' && $loan->status !== 'pending'): ?>
...
<div id="returnModal" class="fixed inset-0 z-[100] hidden items-center justify-center p-6 opacity-0 transition-opacity duration-300">
    ...
    <div class="relative z-10 w-full max-w-md bg-white rounded-2xl p-8 text-center shadow-2xl border border-slate-100 transform scale-95 transition-all duration-300" id="returnModalContent">
        ...
        <p class="text-xs font-bold text-slate-500 leading-relaxed mb-8">
            ...
            <span class="text-slate-800"><?php echo e($loan->book->title); ?></span> oleh <span class="text-slate-800"><?php echo e($loan->user->name); ?></span>?
        </p>
        ...
        <div class="flex items-center gap-4">
            ...
            <form action="<?php echo e(route('admin.loans.return', $loan)); ?>" method="POST" class="flex-1 m-0">
                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\perpustakaan-digital\resources\views/admin/loans/show.blade.php ENDPATH**/ ?>