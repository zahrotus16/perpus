...
<body class="font-sans antialiased text-white min-h-screen flex items-center justify-center relative py-12 px-6">
    ...
    <div class="relative z-20 w-full max-w-[420px]">
        <div class="glass-card rounded-[20px] p-10 text-center fade-in">
            ...
            <div class="flex justify-center mb-8">
                <div class="w-24 h-24 rounded-full bg-white flex items-center justify-center border-4 border-white/20 shadow-xl overflow-hidden">
                   <div class="flex flex-col items-center">
                       <i class="fas fa-shield-halved text-[#0096ff] text-3xl mb-1"></i>
            ...
            <!-- Title & Branding -->
            <div class="mb-10">
                <h1 class="text-xl font-bold tracking-wider uppercase">Atur Ulang Sandi</h1>
            ...
            <form action="{{ route('password.update') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <input type="email" name="email" value="{{ $email ?? old('email') }}" 
                           ...
                           class="input-glass w-full py-3 px-6 rounded-full text-slate-800 text-sm font-medium placeholder-slate-400 focus:outline-none">
                    @error('email')<p class="text-[10px] text-red-200 mt-1 font-bold">{{ $message }}</p>@enderror
                </div>

                <!-- New Password -->
                <div>
                    <input type="password" name="password" 
                           placeholder="Kata Sandi Baru" required autofocus
                           class="input-glass w-full py-3 px-6 rounded-full text-slate-800 text-sm font-medium placeholder-slate-400 focus:outline-none">
                    @error('password')<p class="text-[10px] text-red-200 mt-1 font-bold">{{ $message }}</p>@enderror
                </div>
