...
@section('title', 'Wishlist Personal')
@section('page-title', 'Rencana Baca')
@section('page-subtitle', 'Kurasi koleksi favorit yang ingin Anda eksplorasi')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-4 border-b border-slate-50 dark:border-dark-border/10">
        <h2 class="text-[11px] font-black text-slate-400 dark:text-gray-600 uppercase tracking-[0.25em]">Personal Kurasi: {{ $books->count() }} Koleksi Terpilih</h2>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
        ...
        <div class="group relative bg-white dark:bg-dark-card rounded-3xl shadow-sm dark:shadow-dark-soft border border-slate-100 dark:border-dark-border/20 overflow-hidden transform transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl">
            {{-- Quick Close Action --}}
            <form action="{{ route('user.books.wishlist', $book) }}" method="POST" class="absolute top-4 right-4 z-20 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                @csrf
            ...
            <a href="{{ route('user.books.show', $book) }}" class="block">
                ...
                <div class="aspect-[3/4] relative overflow-hidden">
                    ...
                    @else
                        <div class="w-full h-full flex flex-col items-center justify-center gap-3 bg-slate-50 dark:bg-dark border-b border-white/5 transition-transform duration-700 group-hover:scale-110"
                             style="background: linear-gradient(135deg, {{ $book->category->color }}22, {{ $book->category->color }}55)">
                ...
                <div class="p-6">
                    ...
                    <p class="text-[10px] font-bold text-slate-400 dark:text-gray-600 italic truncate mb-6">{{ $book->author }}</p>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-slate-50 dark:border-dark-border/10">
                        ...
                        <span class="text-[9px] font-black {{ $book->stock > 0 ? 'text-emerald-500' : 'text-red-500' }} uppercase tracking-widest italic">
                            {{ $book->stock > 0 ? 'Unit Available' : 'On Loan' }}
                        </span>
                    </div>
                </div>
            </a>
    ...
    <div class="bg-white dark:bg-dark-card rounded-[3rem] shadow-sm dark:shadow-dark-soft border border-slate-100 dark:border-dark-border/20 py-32 text-center transition-all duration-500 hover:shadow-2xl">
        ...
        <a href="{{ route('user.books.index') }}" class="inline-flex items-center gap-4 bg-primary-600 hover:bg-primary-700 text-white px-12 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition shadow-2xl shadow-primary-900/40 active:scale-95">
