...
@section('page-subtitle', 'Rincian administratif peminjaman buku')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">

    <a href="{{ route('user.loans.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition font-bold uppercase tracking-widest text-[10px]">
    ...
    <div class="bg-white dark:bg-dark-card rounded-2xl shadow-xl dark:shadow-dark-soft border border-slate-100 dark:border-dark-border/20 overflow-hidden">
        ...
        <div class="p-8 border-b border-slate-100 bg-[#aab7c4]/10">
            <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                <div class="text-center md:text-left">
                    ...
                    <p class="font-mono font-black text-slate-800 text-2xl tracking-tighter">{{ $loan->kode_peminjaman }}</p>
                </div>
                <div class="flex flex-col items-center md:items-end">
                    <span class="px-5 py-2 whitespace-nowrap rounded-lg text-[11px] font-black uppercase tracking-widest shadow-sm ring-1 ring-inset
                        {{ $loan->status === 'dipinjam' ? 'bg-[#00b4ff] text-white ring-[#00b4ff]/20' :
                          ($loan->status === 'dikembalikan' ? 'bg-[#2e7d32] text-white ring-[#2e7d32]/20' : 'bg-[#d32f2f] text-white ring-[#d32f2f]/20') }}">
                        {{ $loan->status === 'dipinjam' ? 'Aktif' : ($loan->status === 'dikembalikan' ? 'Arsip' : 'Terlambat') }}
                    </span>
        ...
        <div class="p-8 flex gap-8 items-center border-b border-slate-50 dark:border-dark-border/10">
            <div class="flex-shrink-0 w-20 shadow-lg dark:shadow-dark-soft rounded-xl overflow-hidden ring-1 ring-slate-100 dark:ring-dark-border/10">
                @if($loan->book->cover)
                    <img src="{{ asset('storage/'.$loan->book->cover) }}" alt="" class="w-full aspect-[3/4] object-cover">
                @else
        ...
        <div class="p-8 space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                <div>
                    ...
                    <p class="text-sm font-black text-slate-800 dark:text-white">{{ $loan->tgl_pinjam->translatedFormat('d F Y') }}</p>
                </div>
                <div>
                    <p class="text-[10px] font-black text-slate-400 dark:text-gray-600 uppercase tracking-widest mb-2 flex items-center gap-2">
                        <i class="fas fa-calendar-times text-red-500/50"></i> Jatuh Tempo
                    </p>
                    <p class="text-sm font-black {{ $loan->isOverdue() ? 'text-red-600 dark:text-red-400' : 'text-slate-800 dark:text-white' }}">
                        {{ $loan->tgl_kembali->translatedFormat('d F Y') }}
                    </p>
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm font-black text-emerald-600 dark:text-emerald-400">{{ $loan->pengembalian->tgl_dikembalikan->translatedFormat('d F Y') }}</p>
                </div>
                @endif
            </div>
            ...
            <div class="p-5 bg-red-50 dark:bg-red-950/20 rounded-2xl border border-red-100 dark:border-red-900/30 flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-red-600 dark:text-red-400 font-bold leading-relaxed">
                        Masa pinjaman telah terlampaui selama <span class="bg-red-600 text-white px-2 py-0.5 rounded-lg mx-1">{{ $loan->getDaysOverdue() }} hari</span>. Mohon segera mengembalikan koleksi ini ke perpustakaan pusat untuk menghindari penalti tambahan.
                    </p>
                </div>
            </div>
            @endif
            ...
            <div class="p-5 bg-slate-900 dark:bg-dark border border-slate-800 rounded-2xl flex items-center justify-between shadow-xl">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-lg font-black text-white leading-none">Rp {{ number_format($fine,0,',','.') }}</p>
                    </div>
                </div>
        ...
        <div class="px-8 pb-8 pt-4 flex gap-4">
            @if($loan->book->pdf_file && $loan->status !== 'dikembalikan')
            <a href="{{ route('user.books.read', $loan->book) }}"
            ...
            @endif
            <a href="{{ route('user.books.show', $loan->book) }}"
