...
<head>
    ...
    <title>@yield('title', 'Sistem Pustaka') — Perpustakaan</title>
    ...
    <div class="flex min-h-screen bg-[#f8fafc] overflow-hidden">
        ...
        <div class="flex-1 flex flex-col min-w-0 min-h-screen">
                ...
                @else
                    <header class="h-16 bg-white border-b border-slate-200/60 flex items-center justify-between px-6 lg:px-10 z-40 flex-shrink-0">
                        <div class="flex items-center gap-4">
                            ...
                            <h2 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">@yield('page-title', 'Dashboard')</h2>
                        </div>
                        <div class="flex items-center gap-4">
                            <div class="flex flex-col items-end leading-none hidden sm:flex">
                                <span class="text-[11px] font-black text-slate-800 uppercase">{{ auth()->user()->name }}</span>
                                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">{{ auth()->user()->role_label }}</span>
                            </div>
            ...
            <footer class="h-12 bg-white border-t border-slate-200 flex items-center justify-between px-8 text-slate-400">
                <span class="text-[10px] font-bold uppercase tracking-widest">&copy; {{ date('Y') }} {{ $appSettings['app_name'] ?? 'DigiLib' }}</span>
            </footer>
        </div>
    </div>
