...
<div class="space-y-8 animate-fade-in">
    ...
    {{-- Stats Cards Grid (Modern Minimalist) --}}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <div class="group bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-6 transition-all hover:shadow-xl hover:-translate-y-1">
            <div class="w-16 h-16 rounded-2xl bg-{{ $card['color'] }}-50 text-{{ $card['color'] }}-600 flex items-center justify-center text-2xl shadow-inner border border-{{ $card['color'] }}-100 group-hover:scale-110 transition-transform">
                <i class="fas {{ $card['icon'] }}"></i>
            </div>
            <div>
                <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">{{ $card['label'] }}</p>
                <h3 class="text-2xl font-black text-slate-800 tracking-tight">{{ $card['value'] }}</h3>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div class="flex flex-col gap-8">
        ...
        {{-- Popular Items Section (More Tidy) --}}
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-8 border-b border-slate-50 flex items-center justify-between">
                ...
                <a href="{{ route('admin.books.index') }}" class="px-4 py-2 bg-slate-50 rounded-xl text-[10px] font-black text-slate-500 uppercase tracking-widest hover:bg-[#00bcd4] hover:text-white transition-all">Lihat Semua</a>
            </div>
            <div class="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50/50 hover:bg-white border border-transparent hover:border-slate-100 hover:shadow-lg transition-all group">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-black text-slate-800 truncate">{{ $book->title }}</p>
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">{{ $book->category->name ?? 'Kategori' }}</p>
                    </div>
                    <div class="text-right bg-white px-3 py-1.5 rounded-lg shadow-sm border border-slate-50">
                        ...
                        <p class="text-xs font-black text-[#00bcd4] leading-none">{{ $book->stock }}</p>
                    </div>
                </div>
                @empty
