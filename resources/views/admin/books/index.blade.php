...
@section('page-title', 'Koleksi Perpustakaan')
@section('page-subtitle', 'Daftar buku dan ketersediaan stok koleksi')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.books.create') }}" class="flex items-center justify-center gap-3 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition shadow-xl shadow-emerald-900/20 active:scale-95">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
        <form action="{{ route('admin.books.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-center">
            ...
            @if(request('search'))
                <a href="{{ route('admin.books.index') }}" class="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-red-500 transition-colors">Reset</a>
            @endif
        </form>
    </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50 font-medium whitespace-nowrap">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="px-4 py-4">
                            <span class="text-[10px] text-slate-500 block font-bold uppercase tracking-tight">{{ $book->publisher }}</span>
                            <span class="text-[9px] text-slate-400 font-bold font-mono">{{ $book->year }}</span>
                        </td>
                        <td class="px-4 py-4 text-center">
                            <span class="text-xs font-black text-slate-800">{{ $book->stock }}</span>
                        </td>
                        <td class="px-4 py-4 text-center">
                            @if($book->stock > 0)
                        ...
                        <td class="px-8 py-4">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <button type="button" 
                                        onclick="openDeleteModal('{{ route('admin.books.destroy', $book) }}')"
